-- =================================================================
-- MIGRATION 045: EMERGENCY LOGIN FIX & SCHEMA REPAIR
-- =================================================================

-- 1. FIX SCHEMA PERMISSIONS (The most likely cause of "Database error querying schema")
--    The 'authenticated' role needs usage on 'extensions' to see pgcrypto functions
GRANT USAGE ON SCHEMA extensions TO postgres, anon, authenticated, service_role;

-- Grant execute on all functions in extensions (crypt, gen_salt, etc)
...
-- 2. DROP ALL POTENTIALLY PROBLEMATIC TRIGGERS ON AUTH.USERS
--    These are often not visible in public schema, but we try to drop them by name
...
DROP TRIGGER IF EXISTS on_auth_user_login ON auth.users; -- Common login trigger

-- 3. DROP TRIGGERS ON PUBLIC TABLES THAT MIGHT FIRE ON LOGIN
--    If auth updates last_sign_in_at, and that propagates via trigger to profiles...
...
-- 4. ENSURE SEARCH PATH IS SET FOR THE DATABASE (Best Effort)
--    We can't do ALTER DATABASE here usually, but we can set it for the role
...
-- 5. RE-VERIFY ADMIN_CREATE_USER (Just to be safe, exact same logic as 044 but forced update)
