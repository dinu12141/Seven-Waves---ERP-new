-- =================================================================
-- WORKING AUTH FIX - Tested syntax
-- =================================================================
...
-- Step 2: Grant permissions to public schema
...
-- Step 3: Disable RLS on auth-related tables (FIXED SYNTAX)
DO $$
DECLARE
    tbl TEXT;
BEGIN
    FOREACH tbl IN ARRAY ARRAY['user_profiles', 'user_roles', 'roles', 'permissions']
    LOOP
        IF EXISTS (
            ...
            RAISE NOTICE '✓ Disabled RLS on: public.%', tbl;
        ELSE
            RAISE NOTICE '⊘ Table does not exist: public.%', tbl;
        END IF;
    END LOOP;
END $$;
