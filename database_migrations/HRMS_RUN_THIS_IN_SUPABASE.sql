-- =================================================================
-- ⚡ SEVEN WAVES ERP - COMPLETE HRMS MODULE ⚡
-- RUN THIS IN SUPABASE SQL EDITOR
-- =================================================================
-- Link: https://supabase.com/dashboard/project/mvvuegptxjykhzpatsmn/sql/new
-- 
-- This file combines:
-- 1. 08_hrms_module.sql (Tables & Seed Data)
-- 2. 09_hrms_rpc_functions.sql (RPC Functions)
-- 
-- SAP Business One HANA Standards Compliant
-- Sri Lanka EPF/ETF Compliance
-- =================================================================
...
-- Link departments.manager_id to employees
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Salary Components (Sri Lanka Compliance)
...
-- Leave Types (Sri Lanka Standards)
...
CREATE POLICY employees_hr_policy ON employees
    ...
    USING (
        EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'hr_manager', 'hr_officer', 'director'))
        OR user_id = auth.uid()
    );
...
BEGIN
    ...
    RAISE NOTICE '║     • employees, departments, designations                 ║';
    RAISE NOTICE '║     • salary_components, salary_structures, payroll_entries║';
    RAISE NOTICE '║     • shift_types, attendance, attendance_requests         ║';
    RAISE NOTICE '║     • leave_types, leave_allocations, leave_applications   ║';
    RAISE NOTICE '║     • benefit_types, benefit_claims                        ║';
    RAISE NOTICE '╠═══════════════════════════════════════════════════════════╣';
    RAISE NOTICE '║  🔒 RLS Policies: Configured for role-based access         ║';
    RAISE NOTICE '║  🇱🇰 Sri Lanka Compliance: EPF/ETF configured              ║';
    RAISE NOTICE '║  📦 Seed Data: Salary components, shifts, leave types      ║';
    RAISE NOTICE '╚═══════════════════════════════════════════════════════════╝';
END $$;
